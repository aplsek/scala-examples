...
- the permutations for that element are these sub-permutations, prefixed by the element, plus the empty permutation (important!)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
   //getAnagrams(sentenceOccurrences(sentence))
    getAnagrams2(sentenceOccurrences(sentence),Nil)
  }
  ...
  def getAnagrams(occ: Occurrences) : List[Sentence] = {
    	if (occ.isEmpty) Nil 
    	   ...
    	   for (oc <- com) {
    		   val words = getWords(oc)
    		
    		   for (w <- words) {
    		    // println("word=" + w)
    		     val ocs = subtract(occ,oc)
    		     if (ocs.isEmpty) {
    		        res = res ::: addWord(w,Nil)
    		      //  println("  accept: w =" + w )
    		     }
    		     else {   
    		    	 val anags = getAnagrams(ocs)
    		    	 if (!anags.isEmpty) 
    		    			 res = res ::: addWord(w,anags)
    		     }
    		   }
    	   }
    	   res
    ...
    def getAnagrams2(occ: Occurrences,acc : Sentence) : List[Sentence] = {
    	if (occ.isEmpty) Nil 
    	else {
    	   val com = combinations(occ)
    	   var res =  List[Sentence]()
    	   for (oc <- com) {
    		   ...
    		   for (w <- words) {
    		     ...
    		     if (ocs.isEmpty) {
    		    	val s = w :: acc
    		        res = res ::: List[Sentence](s)
    		     }
    		     else {   
    		    	 val anags = getAnagrams2(ocs, w :: acc)
    		    	 if (!anags.isEmpty) 
    		    			 res = res ::: anags
    		     }
    		   }
  ...
  def addWord(w : Word, sen : List[Sentence]): List[Sentence] = {
    	sen match {
    		case Nil => List[Sentence](List[Word](w))
    		case s :: ss => {
    				List[Sentence](w :: s) ::: addWord(w,ss)
    		}
    	}
  }
  
  def getWords(oc : Occurrences) : List[Word] = {
    if (!dictionaryByOccurrences.contains(oc)) Nil
    else dictionaryByOccurrences(oc)
  }
